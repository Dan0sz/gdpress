				...
				<tbody class="<?php echo esc_attr( $type ); ?>">
					...
					<?php foreach ( $requests as $i => $request ) : ?>
						...
						<tr <?php echo "class='" . esc_attr( $classes ) . "'"; ?>>
							...
							<td class="exclude"><input type="checkbox" <?php echo Helper::is_excluded( $type, $request['href'] ) || $is_ga ? 'checked' : ''; ?> <?php echo $is_ga ? 'class="locked"' : ''; ?> name="<?php echo esc_attr( Settings::GDPRESS_MANAGE_SETTING_EXCLUDED ); ?>[<?php echo esc_attr( $type ); ?>][]" value="<?php echo esc_url( $request['href'] ); ?>" /></td>
						</tr>
					<?php endforeach; ?>
				</tbody>
			<?php endforeach; ?>
