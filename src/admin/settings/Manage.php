...
class Manage extends Builder {
	...
	private function manage_screen() {
		$css_count = isset( Plugin::requests()['css'] ) ? count( Plugin::requests()['css'] ) : 0;
		$js_count  = isset( Plugin::requests()['js'] ) ? count( Plugin::requests()['js'] ) : 0;
		?>
		<?php if ( empty( Plugin::local() ) ) : ?>
			<p>
		...
		<table>
			...
			<?php foreach ( Plugin::requests() as $type => $requests ) : ?>
				<tbody class="<?php echo esc_attr( $type ); ?>">
					...
					<?php foreach ( $requests as $i => $request ) : ?>
						...
						$is_gf = Plugin::is_google_fonts_request( $request['href'] );
						...
						$local_url  = Plugin::is_google_fonts_request( $request['href'] ) ? Plugin::get_local_url_google_font( $request['name'] ) : Plugin::get_local_url( $request['href'], $type );
						$downloaded = file_exists( Plugin::get_local_path( $request['href'], $type ) );
						$descr      = '';
						...
						<tr <?php echo "class='" . esc_attr( $classes ) . "'"; ?>>
							...
							<td class="exclude"><input type="checkbox" <?php echo Plugin::is_excluded( $type, $request['href'] ) || $is_ga ? 'checked' : ''; ?> <?php echo $is_ga ? 'class="locked"' : ''; ?> name="<?php echo esc_attr( Settings::GDPRESS_MANAGE_SETTING_EXCLUDED ); ?>[<?php echo esc_attr( $type ); ?>][]" value="<?php echo esc_url( $request['href'] ); ?>" /></td>
						</tr>
					<?php endforeach; ?>
				</tbody>
			<?php endforeach; ?>
		</table>
		<input type="hidden" name="<?php echo esc_attr( Settings::GDPRESS_MANAGE_SETTING_REQUESTS ); ?>" value="<?php echo base64_encode( serialize( Plugin::requests() ) ); ?>" />
		<?php
	}
	...
	private function start_screen() {
		if ( is_array( Plugin::requests( true ) ) ) : 
			?>
			<p>
		...
		<?php endif; ?>
		<p>
			<button data-nonce="<?php echo wp_create_nonce( Settings::GDPRESS_ADMIN_PAGE ); ?>" id="gdpress-fetch" class="button button-primary button-hero"><?php echo __( 'Scan Website', 'gdpr-press' ); ?></button>
		</p>
		<?php
	}
}
