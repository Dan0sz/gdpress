...
class RewriteUrl {
	...
	public function rewrite_urls( $html ) {
		...
		if ( json_encode( Plugin::requests() ) !== json_encode( $external_reqs ) ) {
	...
	private function parse_stylesheets( $stylesheets, $site_url ) {
		...
		foreach ( $stylesheets as $stylesheet ) {
			...
			if ( function_exists( 'omgf_init' ) && Plugin::is_google_fonts_request( $href ) ) {
				continue;
			}

			$external_css[ $i ]['href'] = $href;

			if ( strpos( $href, '?' ) !== false && ! Plugin::is_google_fonts_request( $href ) ) {
	...
	private function process_requests( $requests, $html ) {
		...
		foreach ( $requests as $type => $type_requests ) {
			foreach ( $type_requests as $request ) {
				...
				if ( Plugin::is_google_fonts_request( $request['href'] ) ) {
					$local_url = Plugin::get_local_url_google_font( $request['name'] );
					$local_dir = Plugin::get_local_path_google_font( $request['name'] );
				} else {
					$local_url = Plugin::get_local_url( $request['href'], $type );
					$local_dir = Plugin::get_local_path( $request['href'], $type );
				}
				...
				if ( ! file_exists( $local_dir ) ) {
					...
					Plugin::set_local_url( $type, $request['href'] );
